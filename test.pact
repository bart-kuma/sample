...
(module fsm-main GOVERNANCE
    ...
    (defconst MAX_SUPPLY:integer 3000)
    ...
    (defun initialize ()
        ...
        (with-capability (GOVERNANCE)
            (insert price-table PRICE_KEY { "price": 0.01})
            ...
            (insert price-table ELECTRIC_FEE { "price" : 0.1 })
            (insert admin-withdraw-table "Riley" { "mined-index" : 0, "key-index" : 1 })
        )
    )

    (defun get-earned-kda-for-ids (reinvested:bool ids:[string] type:integer)
        @doc "Gets the total earned kda for a list of NFTs"
        (fold (+) 0.0 (map (get-earned-kda-for-id reinvested type) ids))
    )
    ...
    (defun get-earned-kda-nft (type:integer id:string)
        ...
        (get-earned-kda-for-id true type id)
    )
    ...
    (defun test-functions ()
        (int-to-str 10 (get-count MINED_TABLE_INDEX))
    )
    ...
    (defun mint (account:string amount:integer)
    	...
    	(let
        	...
        	(insert fsm-nfts id
        		{ "id": id
        		, "uri": (at 'uri (get-nft-collection-uri collection-id))
        		, "owner": account
        		, "attributes": (at 'attributes (get-nft-collection-uri collection-id))
            ...
            (let*
                ...
                (if (>= combined price)
                    (with-capability (PRIVATE)
                        ...
                        (if (>= accrued 0.0)
                            [
                                (update-kda-mined-index (at 'id all-nfts))
                                (add-claim (at 'owner all-nfts) accrued)
                            ]
                            ""
                        )
    ...
    (defun private-get-earned-kda-for-id (reinvested:bool id:string)
        (with-capability (OWNER id)
            (with-read fsm-nfts id
                { 'stake-type := stake-type}
                (get-earned-kda-for-id reinvested stake-type id)
            )
        )
    )
